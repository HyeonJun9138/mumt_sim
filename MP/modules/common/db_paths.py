...
DEFAULT_DB_ROOT = REPO_ROOT / "database"
...
def _set_env_unlocked(db_root: Optional[Path], scenario_dir: Optional[Path]) -> None:
    if db_root is not None:
        ...
        os.environ[ENV_DB_ROOT] = str(DEFAULT_DB_ROOT)
...
def bootstrap_db_root() -> Path:
    with _lock:
        ...
        if env_path:
            ...
            return path
        DEFAULT_DB_ROOT.mkdir(parents=True, exist_ok=True)
        _cache.update({"db_root": DEFAULT_DB_ROOT})
        _set_env_unlocked(DEFAULT_DB_ROOT, None)
