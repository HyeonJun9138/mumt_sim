...
# QtWebEngine 캐시/프로필을 프로젝트 내부로 강제해 AppData 락/권한 오류 회피
_FILE_HERE = Path(__file__).resolve()
_CACHE_ROOT = (_FILE_HERE.parent / "qtwebengine_cache").resolve()
os.environ.setdefault(
    "QTWEBENGINE_CHROMIUM_FLAGS",
    f"--user-data-dir={_CACHE_ROOT} --disk-cache-dir={_CACHE_ROOT / 'cache'} --disable-features=UserAgentClientHint"
)
...
# Repo root (top of workspace) for mission generation, etc.
_REPO_ROOT = Path(__file__).resolve().parents[4]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))
# Ensure FPL_Random/fpl_random is importable (mission generation depends on it)
_FPL_RANDOM = _REPO_ROOT / "FPL_Random"
if str(_FPL_RANDOM) not in sys.path:
    sys.path.insert(0, str(_FPL_RANDOM))


_qt_cache = (PKG_DIR / "qtwebengine_cache").resolve()
(_qt_cache / "storage").mkdir(parents=True, exist_ok=True)
...
_profile.setCachePath(str(_qt_cache))
_profile.setPersistentStoragePath(str(_qt_cache / "storage"))
...
import sys, pathlib
sys.path.insert(0, str(pathlib.Path(__file__).resolve().parents[3] / "FPL_Random"))

from FPL_Random.fpl_random.pipeline import generate_sequence  # auto-generate 0201/0203/targets
...
class MainGUI(QWidget):
    ...
    # ★ 저장 루트: <repo>/database
    SAVE_DIR = _REPO_ROOT / "database"

    # 하위 폴더들
    DIR_0201 = SAVE_DIR / "InputMissionPlan"
    DIR_0203 = SAVE_DIR / "MissionReferenceInfo"
    DIR_TGT  = SAVE_DIR / "TargetInfo"

    def __init__(self) -> None:
        super().__init__()
        # 기본 폴더 보장
        for d in (self.SAVE_DIR, self.DIR_0201, self.DIR_0203, self.DIR_TGT):
    ...
    def _write_map_html(self):
        ...
        folium.Rectangle(
            [[38.0370740,127.2060580], [38.2204690, 127.4299630]],
            color="navy", weight=1, fill=True, fill_opacity=0.1
        ).add_to(fmap)

        # ── 0201 Input Mission (CMPK) ───────────────────────────
        if self._visible_0201 and self._cmpk_data:
            color_area = "#f57c00"  # 영역 가시성 강화 (오렌지 계열)
            color_line = "#00838f"
            color_point = "#1a237e"
        ...
        # TargetInfo overlay
        if self._visible_tgt and self._tgt_data:
            tgt_color = "#c62828"
            for tgt in self._tgt_data.get("targetList") or []:
                loc = tgt.get("location") or {}
                lat = loc.get("latitude"); lon = loc.get("longitude")
                if lat is None or lon is None:
                    continue
                tid = tgt.get("targetID", "-")
                ttype = tgt.get("targetType", "-")
                mid = tgt.get("inputMissionID", "-")
                popup = folium.Popup(
                    _html_kv("Target", {
                        "targetID": tid,
                        "type": ttype,
                ...
                folium.CircleMarker(
                    [lat, lon], radius=5, color=tgt_color,
                    fill=True, fill_opacity=0.9,
                    popup=popup,
                    tooltip=f"TGT {tid} (type {ttype})",
                ).add_to(fmap)
        ...
        if self._visible_0203 and self._mrpk_data:
            ...
            for idx_hand, item in enumerate(mrpk.get("handOverInfoList") or [], 1):
                ...
                dist = None
                if idx_hand - 1 < len(take_pts):
                    tlat, tlon, _ = take_pts[idx_hand - 1]
                    dist = _haversine_m(tlat, tlon, lat, lon)
                popup = folium.Popup(
                    _html_kv("0203 Hand-Over", {
                        "aircraft": aid,
                        "alt(m)": coord.get("altitude"),
                        "dist_from_take(m)": f"{dist:.1f}" if dist is not None else "-",
                    }), max_width=240
                )
                folium.CircleMarker(
            ...
            for idx_rtb, coord in enumerate(mrpk.get("rtbCoordinateList") or [], 1):
                ...
                dist = None
                if idx_rtb - 1 < len(take_pts):
                    tlat, tlon, _ = take_pts[idx_rtb - 1]
                    dist = _haversine_m(tlat, tlon, lat, lon)
                popup = folium.Popup(
                    _html_kv("0203 RTB", {
                        "index": idx_rtb,
            ...
            for idx_proh, area in enumerate(mrpk.get("prohibitedAreaList") or [], 1):
                ...
                if len(coords) < 3:
                    continue
                poly = folium.Polygon(
                    coords, color="#d32f2f", weight=2.0,
...
# ─────────────────────────── 초기임무계획 탭 ───────────────────────────
    def _build_tab_initial(self):
        ...
        # ------------- Load TargetInfo --------------------------------
        btn_tgt = QPushButton("Load TargetInfo")
        btn_tgt.clicked.connect(self._load_target_json)
        form.addRow(btn_tgt)

        # ------------- Auto-generate 0201/0203 (+Target) ------------
        self.spn_autogen_cnt = QSpinBox()
        self.spn_autogen_cnt.setRange(1, 20)
        self.spn_autogen_cnt.setValue(1)
        btn_autogen = QPushButton("Auto-generate 0201/0203 (+Target)")
        btn_autogen.clicked.connect(self._auto_generate_inputs)
        row_auto = QHBoxLayout()
        row_auto.addWidget(btn_autogen)
        row_auto.addWidget(QLabel("Count:"))
        row_auto.addWidget(self.spn_autogen_cnt)
        form.addRow(row_auto)
        ...
        btn_tgt_vis = QPushButton("TGT")
        btn_tgt_vis.setCheckable(True); btn_tgt_vis.setChecked(True)
        btn_tgt_vis.setStyleSheet("QPushButton:checked{background:#4caf50;color:white}"
                                  "QPushButton{background:#ddd}")
        btn_tgt_vis.toggled.connect(lambda state: self._toggle_dataset("TGT", state))
        vis_row.addWidget(btn_tgt_vis)
    ...
    def _toggle_dataset(self, tag: str, state: bool):
        if tag == "0201":
            ...
            self._visible_tgt = state
        else:
            return
        self._rebuild_map()

    def _auto_generate_inputs(self):
        """
        Run fpl_random pipeline to create 0201/0203(+targets) and load them.
        """
        cnt = self.spn_autogen_cnt.value() if hasattr(self, "spn_autogen_cnt") else 1
        base_seed = int(time.time() * 1000) % 1_000_000_000
        last_paths = None
        try:
            for i in range(cnt):
                res = generate_sequence(seed=base_seed + i, save=True)
                paths = res.get("paths", {})
                last_paths = paths
                self.log_init.appendPlainText(
                    f"[OK] Auto-gen #{i+1}/{cnt} → 0201={paths.get('input_mission_plan')} / "
                    f"0203={paths.get('flight_reference')} / tgt={paths.get('targets')}"
                )
        except Exception as e:
            QMessageBox.warning(self, "Auto-generate 실패", str(e))
            return

        if not last_paths:
            return

        cmpk_path = last_paths.get("input_mission_plan")
        mrpk_path = last_paths.get("flight_reference")
        tgt_path = last_paths.get("targets")

        if not self._load_0201_from_path(cmpk_path, warn=True):
            return
        self._load_0203_from_path(mrpk_path, warn=True)
        if tgt_path:
            self._load_target_from_path(tgt_path, warn=False)
    ...
    def _generate_all(self):
        ...
        # 자동으로 결과 파일을 기본 DB 경로에 저장
        try:
            self._save_all_missions()
            self.log_init.appendPlainText("[OK] DB 디렉터리에 0301/0302/0303/0304 저장 완료")
        except Exception as e:
            self.log_init.appendPlainText(f"[WARN] 결과 저장 실패: {e}")
    ...
    # ─────────────────── 0201 / 0203 Load 유틸 ───────────────────
    def _load_0201_from_path(self, path: str, warn: bool = True) -> bool:
        if not path:
            return False
        try:
            with open(path, encoding="utf-8") as f:
            ...
            if not m:
                raise ValueError("파일 이름 끝에 번호가 필요합니다. 예) LAHMUMT-IMP-0001.json")
            self.pkg0201_id = int(m.group(1))
        except Exception as e:
            if warn:
    ...
    def _load_0201_json(self):
        ...
        if not path:
            return
        self._load_0201_from_path(path, warn=True)

    def _load_0203_from_path(self, path: str, warn: bool = True) -> bool:
        if not path:
            return False
        try:
            with open(path, encoding="utf-8") as f:
            ...
            m = re.search(r"(\d+)$", Path(path).stem)
            if not m:
                raise ValueError("파일 이름 끝에 번호가 필요합니다. 예) LAHMUMT-SCN-0001.json")
            self.pkg0203_id = int(m.group(1))
        except Exception as e:
            if warn:
    ...
    def _load_0203_json(self):
        ...
        if not path:
            return
        self._load_0203_from_path(path, warn=True)

    def _load_target_from_path(self, path: str, warn: bool = True) -> bool:
        if not path:
            return False
        try:
            with open(path, encoding="utf-8") as f:
            ...
            if warn:
                QMessageBox.warning(self, "Target 로드 실패", str(e))
            return False
        self._tgt_path = path
        self._tgt_data = data
        self.log_init.appendPlainText(f"[OK] TargetInfo loaded  →  {path}")
        self._rebuild_map()
        return True

    def _load_target_json(self):
        path, _ = QFileDialog.getOpenFileName(
            self,
            "TargetInfo JSON",
            str(self.DIR_TGT),
        ...
        if not path:
            return
        self._load_target_from_path(path, warn=True)
