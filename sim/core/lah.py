"""
LAH helicopter model: operational envelope + simple 6-DoF-ish kinematics.

This prepares a second airframe type (rotary wing) alongside the existing UAV.
Currently it is not wired into `main.py`; you can instantiate and integrate it
later to swap airframes.
"""
...
class LAHOperationalEnvelope:
    """
    High-level operating limits for the LAH helicopter.
    """

    # Speed / maneuver limits
...
class LAHParams:
    """
    Low-level control limits tuned for helicopter-like response.
    """

    max_yaw_rate_dps: float = 45.0
    max_pitch_rate_dps: float = 20.0
    max_roll_rate_dps: float = 30.0
    max_speed: float = 75.0  # ≈270 km/h
    min_speed: float = 0.0   # hover-capable
    accel: float = 6.0
...
class LAH:
    """
    Simplified dynamics for a rotary-wing airframe.
    Hover-capable, slower roll/pitch rates, and allows zero forward speed.
    """
    ...
    def __init__(self, params: LAHParams):
        ...
        self.cmd_throttle = 0.0  # -1..1: descent..climb
    ...
    def step(self, dt: float):
        ...
        self.s.x += vx * dt
        self.s.y += vy * dt
        # Helicopter: allow negative vz when throttle < 0 and pitch small
    ...
    def cmd_straight(self):
        # For compatibility with fixed-wing input mapping; hover in place.
        self.cmd_hover()
